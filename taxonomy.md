# Intent Taxonomy -- v1 (as built) and v2 (proposed)

This document holds **both** taxonomies: v1 exactly as it is
implemented today, and v2 as proposed to replace it, with the reasoning
and evidence for every change.

- **v1 -- 8 intents.** Live. Used by `intents.json`, `groq_lib.py`, and
  every classification in `data/classification_cache.jsonl`.
  `intents.md` is the original human-readable record; Part 1 below
  restates it so this document stands alone.
- **v2 -- 13 intents + `turn_type` + 8 flags.** Proposed, not adopted.
  No code uses it yet.
